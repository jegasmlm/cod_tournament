...
function Position({playerScore, place}) {
  return (
    <div className={`place ${place === 1 ? 'first-place' : place === 2 ? 'second-place' : 'third-place'} ml mr`}>
      <PlayerItem size='lg' player={playerScore.player} />
      <div className='box v-layout'>
        <h1 className='text-white m0'>{place}<span className='text-sm'>{place === 1 ? 'ft' : place === 2 ? 'nd' : 'rd'}</span></h1>
